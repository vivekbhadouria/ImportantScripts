...
"""
Vivek
This scripts has been tested with OpenCV 2.9. It will throw an error with OpenCV 3 (cv2.CV_AA will be the cause of error)
"""
...
def demo(net, image_name):
    ...
    # print os.getcwd()
    cap = cv2.VideoCapture('Ferrari.mp4')
    ...
    while(cap.isOpened()):
        ...
        if ret:
           ...
           for cls_ind, cls in enumerate(CLASSES[1:]):
               ...
               for i in inds:
                   bbox = dets[i, :4]
                   # Vivek. Adding different color boxes for different object
                   # In place of "car" or "tyre", we can put cls for a generic visualization
                   if cls == "n02974003":
                       cv2.putText(frame, "Tyre", (bbox[0], int(bbox[3] + 25)), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2, cv2.CV_AA)
                       cv2.rectangle(frame, (bbox[0], bbox[1]), (bbox[2], bbox[3]), (255,255,0), 3)
                   if cls == "n04037443":
                       cv2.putText(frame, "Car", (bbox[0], int(bbox[3] + 25)), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2, cv2.CV_AA)
           ...
           #cv2.imwrite("frame%d.jpg" % countImg, frame)
           countImg = countImg + 1
...
def parse_args():
    ...
    parser.add_argument('--videoname', dest='videoname', help='Video Name', default='')
